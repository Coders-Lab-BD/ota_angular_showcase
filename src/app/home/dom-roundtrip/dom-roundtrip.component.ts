...
@Component({
  selector: 'app-dom-roundtrip',
  templateUrl: './dom-roundtrip.component.html',
  styleUrls: ['./dom-roundtrip.component.css']
})
export class DomRoundtripComponent implements OnInit {
  isInstant: any;
  ...
  bookAndHoldAction()
  {
    ...
    let flightIndi=this.summaryBarLegData;
    var agentfare = flightIndi.firstLegData[0].agentFareTotal + flightIndi.secondLegData[0].agentFareTotal;
    if(amount >= agentfare && this.isInstant == true){
      // console.log("Flight Individuals:");
        ...
        try{
          this.authService.getFlightSearch(Object.assign({},this.fmgFlightSearch.value)).subscribe( data=>{
            ...
            if(!this.shareService.isObjectEmpty(data))
            {
              ...
              if(!this.shareService.isObjectEmpty(data.flightDataTwo)
              && !this.shareService.isObjectEmpty(data.flightDataTwo[0].groupedItineraryResponse))
              {
          ...
          // if(this.isNotFound) console.log("2");
        }
      }
    }, 1000);
  }
  private _setBookInstantEnableDisable(data:any)
  {
    ...
    if(this.bookInstantEnableDisable.length>0)
    {
      ...
      this.isNotFound=true;
      // if(this.isNotFound) console.log("3");
    }
  }
  private _setMarkupDiscountDetails(data:any)
  {
    ...
    if(this.markupInfo.length>0 && this.discountInfo.length>0)
    {
      ...
      this.isNotFound=true;
      // if(this.isNotFound) console.log("4");
    }
  }
  _flightWork()
  {
  ...
  setFlightData()
  {
    ...
    try {
        ...
        for(let flightItem of this.selectedFlightArrival)
        {
          if(rootItem.groupDescription.legDescriptions[0].arrivalLocation==flightItem.CityCode)
          {
              for(let itiItem of rootItem.itineraries)
              {
                ...
                let fInd=0;
                let dadj = 0;
                let index=this.flightDataFirstLeg.findIndex(x=>x.id===itiItem.id && x.providerName===itiItem.nvProviderName);
                let adjustAct=0;
                for(let item of this._schedules(ref))
                {
                  ...
                  this.flightDataFirstLeg[index].flightSegmentData.push({
                    ...
                    airlineCode:airlineCode,
                    airlineId:airlineId,
                    providerName:this.flightDataFirstLeg[index].providerName,
                    supplierID:this.flightDataFirstLeg[index].supplierID,
                    routeWiseMarkUpDiscountDetailsID:this.flightDataFirstLeg[index].routeWiseMarkUpDiscountDetailsID,
                    ticketIssueType:this.flightDataFirstLeg[index].ticketIssueType,
                    ticketIssueTypeCommission:this.flightDataFirstLeg[index].ticketIssueTypeCommission,
                  ...
                  this.flightDataFirstLeg[index].depadjustment=adj>depAdj?adj:depAdj;
                  this.flightDataFirstLeg[index].arradjustment=adj>arrAdj?adj:depAdj>arrAdj?depAdj:arrAdj;
      ...
      }catch(exp)
      {
        console.log(exp);
        this.isNotFound=true;
        // if(this.isNotFound) console.log("5");
      }
  }
  ...
  setFlightDataTwo()
  {
    ...
    try {
      ...
      {
        for(let flightItem of this.selectedFlightDeparture)
        {
          if(rootItem.groupDescription.legDescriptions[0].arrivalLocation==flightItem.CityCode)
          {
              for(let itiItem of rootItem.itineraries)
              {
                ...
                let fInd=0;
                let dadj = 0;
                let index=this.flightDataSecondLeg.findIndex(x=>x.id===itiItem.id && x.providerName===itiItem.nvProviderName);
                let adjustAct=0;
                for(let item of this._schedulesTwo(ref))
                {
                  ...
                  let fDifTime=this._timeDifferenceActualTwo1(item.ref);
                  ...
                  this.flightDataSecondLeg[index].depadjustment=adj>depAdj?adj:depAdj;
                  this.flightDataSecondLeg[index].arradjustment=adj>arrAdj?adj:depAdj>arrAdj?depAdj:arrAdj;
      ...
      {
        this.isNotFound=true;
        // if(this.isNotFound) console.log("6");
      }
  }
    ...
    try{
      if(type=="Yes")
      {
        if(event.target.checked)
        {
          ...
          {
            this.addAppliedFilterItem("fa fa-times-circle-o","","",`Refundable -
            ${this.selectedFlightDeparture[0].CityCode} to ${this.selectedFlightArrival[0].CityCode}`,"Refundable","0");
            this.refundFilterList.push(true);
            $("#chkRefundNo").prop("checked",false);
            this.removeAppliedFilterItem("NonRefundable");
          }
      ...
      if(type=="No")
      {
        if(event.target.checked)
        {
          ...
          {
            this.addAppliedFilterItem("fa fa-times-circle-o","","",`NonRefundable -
            ${this.selectedFlightDeparture[0].CityCode} to ${this.selectedFlightArrival[0].CityCode}`,"NonRefundable","1");
    ...
    try{
      if(type=="Yes")
      {
        if(event.target.checked)
        {
          ...
          {
            this.addAppliedFilterItem("fa fa-times-circle-o","","",`Refundable - ${this.selectedFlightArrival[0].CityCode} to
            ${this.selectedFlightDeparture[0].CityCode}`,"RefundableTwo","0");
            this.refundFilterList.push(true);
            $("#chkRefundNoTwo").prop("checked",false);
      ...
      if(type=="No")
      {
        if(event.target.checked)
        {
          ...
          {
            this.addAppliedFilterItem("fa fa-times-circle-o","","",`NonRefundable -
            ${this.selectedFlightArrival[0].CityCode} to ${this.selectedFlightDeparture[0].CityCode}`,"NonRefundableTwo","1");
            this.refundFilterList.push(false);
            $("#chkRefundYesTwo").prop("checked",false);
            this.removeAppliedFilterItem("RefundableTwo");
          }
  ...
  showFlightDetailsMobile(ind:any)
  {
    ...
    try{
      ...
      // this.flightDetailsModalData[0].flightDataFirstLeg.push(data.flightSegmentData);
  ...
  {
    try{
      let data={firstLegData:this.summaryBarLegData.firstLegData,
      secondLegData:this.summaryBarLegData.secondLegData,
      domestic:true,tripTypeId:this.summaryBarLegData.firstLegData[0].tripTypeId
    };
  ...
  _timeDifferenceActualTwo1(legs:any):string {
    var td = "", data
    try{
          data = this.rootDataTwo.scheduleDescs.find( (x: { id: number; })=>x.id==legs).elapsedTime;
         let h = Math.floor(data/60);
         let m = data%60;
         td = h.toString() + 'h ' + m.toString() + 'm'  
        return td;
  ...
  _pricingInfoTwo(ind:any):any{
    ...
    try{
      for(let rootItem of this.rootDataTwo.itineraryGroups)
      {
        if(rootItem.groupDescription.legDescriptions[0].arrivalLocation==this.selectedFlightDeparture[0].CityCode)
        {
          for(let item of rootItem.itineraries)
          {
            if(item.id===ind)
  ...
  setDepartureTimeFilterTwo(type:string,a_p:any,hours:any,time:any)
  {
    try{
      if(a_p.toString().indexOf('AM')>-1)
      {
        ...
        {
          this.departureTimeFilterTwo.push({title:"Before 06AM",details:"12:00 AM-05:59 AM",value:"12AM-06AM",
          price:before6,logo:'../../../assets/dist/img/sun.svg'});
  ...
  setStopCountTwo()
  {
    ...
    try{
      ...
      for(let item of stopGroup)
      {
        let min=item.value[0].price;
        let title=item.key==1?"Non stop":"Stop "+(item.key-1);
        this.stopCountListTwo.push({id:item.key,stopCount:item.value.length,title:title,
        price:0, idTwo:11});
        for(let subItem of item.value)
        {
          if(min>subItem.price)
    ...
    setTimeout(()=>{
      this.airportsTwo={departure:[],arrival:[]};
        ...
        for(let i=0;i<this.cmbAirportTwo.length;i++)
        {
          ...
          {
            this.airportsTwo.departure.unshift({depCode:this.cmbAirportTwo[i].code,depName:this.cmbAirportTwo[i].text,
        ...
        for(let i=0;i<this.cmbAirportTwo.length;i++)
        {
          ...
          {
            this.airportsTwo.arrival.unshift({arrCode:this.cmbAirportTwo[i].code,arrName:this.cmbAirportTwo[i].text,
...
setAirlineList() {
  this.authService.getAirlineInfo().subscribe(data => {
    ...
    // console.log(this.rootDataTwo.itineraryGroups)
        ...
        for(let subItem of airlineData)
        {
          ...
          for(let item of scheduleAriline)
          {
            ...
            )
            {
              ...
              this.airlines.push({id:subItem.vAirlinesId,
    ...
    for(let rootItem of this.rootDataTwo.itineraryGroups)
    {
      // have to check selected airport to code
      if(rootItem.groupDescription.legDescriptions[0].arrivalLocation==this.selectedFlightDeparture[0].CityCode)
      {
        for(let subItem of airlineData)
        {
          ...
          for(let item of scheduleAriline)
          {
            ...
            )
            {
              ...
              this.airlinesTwo.push({id:subItem.vAirlinesId,
                code:subItem.nvIataDesignator,logo:subItem.vLogo,
                number:subItem.nvAirlinesCode,
                name:subItem.nvAirlinesName,len:0});
            }
    ...
    // have to create two method for two schedule description
    this.setAirport(this.scheduleDescs);

    // For Schedule Description Two
...
getArrCountryName(obj:string):string{
  ...
  legDataSet1(item:any,type:number,provider:any) {
    if(type==1)
    {
      this.summaryBarLegData.firstLegData=[];
      let d1=this.flightDataFirstLeg.find(x=>x.id==item && x.providerName==provider);
      this.summaryBarLegData.firstLegData.push(d1);
    }
    if(type==2)
    {
      this.summaryBarLegData.secondLegData=[];
      let d2=this.flightDataSecondLeg.find(x=>x.id==item && x.providerName==provider);
      this.summaryBarLegData.secondLegData.push(d2);
    }
  }
